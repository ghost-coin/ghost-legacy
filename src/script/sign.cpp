...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    bool fIsP2SH = creator.IsGhostVersion()
    ...
    if (creator.IsGhostVersion() && !sigdata.witness) {
        sigdata.scriptWitness.stack = result;
    } else  {
...
public:
    ...
    bool is_ghost_tx = false;
    bool IsGhostVersion() const override { return is_ghost_tx; }
};
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const std::vector<uint8_t> &amount, const CScript &scriptPubKey)
{
    ...
    extractor_checker.is_ghost_tx = tx.IsGhostVersion();
...
class DummySignatureCheckerGhost : public DummySignatureChecker
{
// IsGhostVersion() must return true to skip stack evaluation
public:
    DummySignatureCheckerGhost() : DummySignatureChecker() {}
    bool IsGhostVersion() const override { return true; }
};
const DummySignatureCheckerGhost DUMMY_CHECKER_GHOST;

class DummySignatureCreatorGhost : public DummySignatureCreator {
public:
    DummySignatureCreatorGhost() : DummySignatureCreator(33, 32) {}
    const BaseSignatureChecker& Checker() const override { return DUMMY_CHECKER_GHOST; }
    bool IsGhostVersion() const override { return true; }
};
...
const BaseSignatureCreator& DUMMY_SIGNATURE_CREATOR_GHOST = DummySignatureCreatorGhost();
