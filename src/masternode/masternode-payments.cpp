...
bool CMasternodePayments::IsTransactionValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward) const
{
    ...
    //! modified to suit ghost/particl tx
    for (const auto& txout : voutMasternodePayments) {
        bool found = false;
        for (const auto& txout2 : txNew.vpout) {
            if (txout2->IsType(OUTPUT_STANDARD) &&
                txout.scriptPubKey == *txout2->GetPScriptPubKey() &&
                txout.nValue == txout2->GetValue()) {
                found = true;
                break;
            }
        }
