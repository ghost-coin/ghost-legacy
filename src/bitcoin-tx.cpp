...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    fGhostMode = !gArgs.GetBoolArg("-btcmode", false);
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > CTransaction::MAX_STANDARD_GHOST_VERSION)
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");

    if (!tx.IsGhostVersion() && IsGhostTxVersion(newVersion)) {
