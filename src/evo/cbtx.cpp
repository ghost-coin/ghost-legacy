...
{
    if (tx.GetType() != TXN_COINBASE && tx.GetType() != TXN_COINSTAKE) {
...
{
    if (block.vtx[0]->GetType() != TXN_COINBASE) {
        return true;
    }
    ...
    if (quorums == quorumsCached) {
        ...
        for (const auto& p : quorums) {
            ...
            for (const auto& p2 : p.second) {
                ...
                if (!found) return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "commitment-not-found");
