...
#include <amount.h>
#include <key_io.h> // GetAddrFromTxOut for debug
#include <chain.h>
...
bool CColdRewards::UpdateWithBlock(const Config &config, CBlockIndex *pindexNew) {
  ...
  CAmount minRewardBalance = consensusParams.getMinRewardBalance(nHeight);
  ReadBlockFromDisk(block, pindexNew);
    ...
    if (!tx->IsCoinBase()) {
      ...
      for (const CTxOut &out : tx->vout) {
        ...
        if (balance >= minRewardBalance) {
          LogPrint(BCLog::COLDREWARD, "CR: %s : Writing to Rewards db, addr %s, Value of %d at Height %d\n", __func__,
      ...
      for (const CTxIn &in : tx->vin) {
        ...
        if (pdb->HaveReward(outpoint)) {
          ...
          if (!pdb->GetReward(outpoint, coinr)) {
            LogPrint(BCLog::COLDREWARD, "CR: %s : Problem getting coin from Rewards db at Height %d, value %d\n", __func__,
    ...
    if (!tx->IsCoinBase()) {
      ...
      for (const CTxIn &in : tx->vin) {
        ...
        if (prewardsdb->HaveReward(out)) {
          ...
          LogPrint(BCLog::COLDREWARD, "CR: %s : Add to rewardUpdates %s\n", __func__, val.ToString());
        ...
        if (prewardsdb->HaveReward(outpoint)) {
          {
            ...
            LogPrint(BCLog::COLDREWARD, "CR: %s : Add to rewardRemovals %s\n", __func__, val.ToString());
          }
...
bool CColdRewards::FindReward(const Consensus::Params &consensusParams, int Height, CTxOut &rewardPayment) {
  ...
  CAmount minRewardBalance = consensusParams.getMinRewardBalance(Height);
  ...
  while (pcursor->Valid()) {
    ...
    if (!pcursor->GetValue(the_reward)) { LogPrint(BCLog::COLDREWARD, "CR: %s: cannot parse CCoins record", __func__); }
    ...
    if (the_reward.IsActive() & !remove_lowrewards) {
      ...
      if (nHeight <= minHeight) { // same Height OK to check for bigger rewards
        ...
        if (HeightDiff > nMinBlocks) {
          ...
          CAmount reward = CalculateReward(consensusParams, nHeight, HeightDiff, balance);
          // LogPrint(BCLog::COLDREWARD, "CR: %s : Candidate : %s, Reward %d\n", __func__, coinreward.ToString(), reward);
      ...
      if (remove_lowrewards) {
        LogPrint(BCLog::COLDREWARD, "CR: %s : Dropping Candidate %s : %s, Reward %d\n", __func__, the_reward.ToString());
        pdb->EraseReward(key);
      }
...
bool CColdRewards::CheckReward(const Consensus::Params &consensusParams, int Height, CTxOut &rewardPayment) {
  ...
  while (pcursor->Valid()) {
    ...
    interruption_point(ShutdownRequested());
    if (!pcursor->GetKey(key)) { break; }
    if (!pcursor->GetValue(val)) { LogPrint(BCLog::COLDREWARD, "CR: %s: cannot parse CCoins record", __func__); }
    ...
    if (val.IsActive() && (HeightDiff > nMinBlocks) && (script_ref == val.scriptPubKey())) {
      const CAmount balance = val.GetValue();
      CAmount reward = CalculateReward(consensusParams, nHeight, HeightDiff, balance);
...
//
void CColdRewards::ClearLowRewards(const Consensus::Params &consensusParams, const CAmount minRewardBalance) {
...
bool CColdRewards::RestoreRewardAtHeight(int Height) {
  ...
  while (pcursor->Valid()) {
    ...
    if (nHeight == Height) { // Bingo!
      ...
      LogPrint(BCLog::COLDREWARD, "CR: %s : Restore Reward At %s Height %d\n", __func__, the_reward.ToString(), Height);
      return true;
    }
    //
    pcursor->Next();
...
void CColdRewards::UpdateRewardsDB(int nNewHeight) {
  ...
  LogPrint(BCLog::COLDREWARD, "CR: %s : %s\n", __func__, coinreward.ToString());
  pdb->EraseReward(rewardKey);

  const Consensus::Params consensusParams = GetConfig().GetChainParams().GetConsensus();
  CAmount minRewardBalance = consensusParams.getMinRewardBalance(nNewHeight);
...
//
CTxOut CColdRewards::GetPayment(const CRewardValue &coinreward, CAmount reward) {
  CTxOut out = CTxOut(reward, coinreward.txout.scriptPubKey);
  return out;
}

// Validate!
bool CColdRewards::FullValidate(const Consensus::Params &consensusParams, const CBlock &block, int nHeight, CAmount &reward,
                            bool fJustCheck) {
...
bool CColdRewards::QuickValidate(const Consensus::Params &consensusParams, const CBlock &block, int nHeight,
                                 CAmount &reward, bool fJustCheck) {
  ...
  if (nHeight == clearLowRewardsSuperBlock*nBlocksPerPeriod + 1) {
        const CAmount minRewardBalance = consensusParams.getMinRewardBalance(nHeight);
