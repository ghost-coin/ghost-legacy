...
CAmount CalculateReward(const Consensus::Params &consensusParams, int Height, int HeightDiff, CAmount balance) {
  ...
  CAmount nMinReward = consensusParams.nMinReward;
  int64_t balance_int = balance;
  ...
  int64_t reward_x100 = (int64_t(100) * HeightDiff * reward_per_block) / COIN;
  // Now compensate for multiply by 100 and divide by COIN
  CAmount reward = reward_x100 * (COIN / 100);
  if (reward < nMinReward) reward = CAmount();
  return reward;
}
