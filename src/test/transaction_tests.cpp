...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    t.vin.clear();
    t.vin.resize(2438 + 2439 /* Part */); // size per input (empty scriptSig): 41 bytes
    ...
    // increase output size by one byte, so we end up with 400004 vbytes (two bytes for Particl)
