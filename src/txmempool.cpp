...
void CTxMemPool::addAddressIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++)
    {
        ...
        CMempoolAddressDelta delta(entry.GetLegacyTime(), nValue * -1, input.prevout.hash, input.prevout.n); //! TODO - convert to chrono::seconds
    ...
    for (unsigned int k = 0; k < tx.vpout.size(); k++)
    {
        ...
        mapAddress.insert(std::make_pair(key, CMempoolAddressDelta(entry.GetLegacyTime(), nValue))); //! TODO - convert to chrono::seconds
        inserted.push_back(key);
    };
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    if (it->GetTx().GetType() == TRANSACTION_PROVIDER_REGISTER) {
        CProRegTx proTx;
    ...
    } else if (it->GetTx().GetType() == TRANSACTION_PROVIDER_UPDATE_SERVICE) {
        CProUpServTx proTx;
        if (!GetTxPayload(it->GetTx(), proTx)) {
            assert(false);
        }
        ...
        mapProTxAddresses.erase(proTx.addr);
    } else if (it->GetTx().GetType() == TRANSACTION_PROVIDER_UPDATE_REGISTRAR) {
        CProUpRegTx proTx;
        if (!GetTxPayload(it->GetTx(), proTx)) {
            assert(false);
        }
        eraseProTxRef(proTx.proTxHash, it->GetTx().GetHash());
        mapProTxBlsPubKeyHashes.erase(proTx.pubKeyOperator.GetHash());
    } else if (it->GetTx().GetType() == TRANSACTION_PROVIDER_UPDATE_REVOKE) {
        CProUpRevTx proTx;
        if (!GetTxPayload(it->GetTx(), proTx)) {
            assert(false);
        }
