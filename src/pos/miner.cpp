...
void StartThreadStakeMiner()
{
    ...
    if (!gArgs.GetBoolArg("-staking", true)) {
        ...
        for (size_t i = 0; i < nThreads; ++i) {
            ...
            GetGhostWallet(vpwallets[i].get())->nStakeThread = i;
...
void ThreadStakeMiner(size_t nThreadID, std::vector<std::shared_ptr<CWallet>> &vpwallets, size_t nStart, size_t nEnd)
{
    ...
    bool fShouldBypasspeercheck = gArgs.GetChainName() == CBaseChainParams::REGTEST || gArgs.GetChainName() == CBaseChainParams::TESTNET;
    ...
    while (!fStopMinerProc) {
        ...
        for (size_t i = nStart; i < nEnd; ++i) {
            auto pwallet = GetGhostWallet(vpwallets[i].get());
