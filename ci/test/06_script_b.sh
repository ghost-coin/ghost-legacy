...
if [ "$RUN_FUNCTIONAL_TESTS" = "true" ]; then
  ...
  DOCKER_EXEC LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib ${TEST_RUNNER_ENV} test/functional/test_runner.py --ci $MAKEJOBS --tmpdirprefix "${BASE_SCRATCH_DIR}/test_runner/" --ansi --combinedlogslen=4000 ${TEST_RUNNER_EXTRA} --quiet --failfast --particl --insight ${TEST_RUNNER_BITCOIN}
  END_FOLD
fi
