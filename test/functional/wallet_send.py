...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for mode in ["unset", "economical", "conservative", "part/kb", "sat/b"]:
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0.00007, estimate_mode="part/kb", add_to_wallet=False)
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_conf_target=0.00004531, arg_estimate_mode="part/kb", add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        ...
        for mode in ["part/kb", "sat/b"]:
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0, estimate_mode=mode,
                expect_error=(-4, "Fee rate (0.00000000 PART/kB) is lower than the minimum fee rate setting (0.00001000 PART/kB)"))
        ...
        for unit, fee_rate in {"sat/B": 0.99999999, "PART/kB": 0.00000999}.items():
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=fee_rate, estimate_mode=unit,
                expect_error=(-4, "Fee rate (0.00000999 PART/kB) is lower than the minimum fee rate setting (0.00001000 PART/kB)"))

        self.log.info("Explicit fee rate raises RPC error if estimate_mode is passed without a conf_target")
        for unit, fee_rate in {"sat/B": 100, "PART/kB": 0.001}.items():
