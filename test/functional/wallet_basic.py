...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Selected estimate_mode pArt/kB requires a fee rate to be specified in conf_target",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            estimate_mode='pArt/kB')
        ...
        for unit, fee_rate in {"PART/kB": 0.00000999, "sat/B": 0.99999999}.items():
            self.log.info("Test sendmany raises 'fee rate too low' if conf_target {} and estimate_mode {} are passed".format(fee_rate, unit))
            assert_raises_rpc_error(-6, "Fee rate (0.00000999 PART/kB) is lower than the minimum fee rate setting (0.00001000 PART/kB)",
